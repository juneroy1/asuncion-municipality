@extends('layouts.master_admin', ['title' => 'legislative_department', 'subtitle' => ''])
        ...
        <div class="page-breadcrumb">
            <div class="row align-items-center">
                <div class="col-md-6 col-8 align-self-center">
                    <h3 class="page-title mb-0 p-0">{{ $edit ? 'Update Legislative Agenda' : 'Create  Legislative Agenda' }}</h3>
                    <div class="d-flex align-items-center">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="#">Home</a></li>
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ $edit ? 'Update  Legislative Agenda' : 'Create  Legislative Agenda' }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
        ...
        <div class="container-fluid">
            ...
            @if (\Session::has('error'))
                ...
                @if ($department != 'super_admin')
                    <form method="POST"
                        action="{{ $edit ? '/admin-manage-legislative-file-update/' . $update->id : '/admin-manage-legislative-file' }}"
                            ...
                            <div class="card">
                                <div class="card-body profile-card">
                                    <center class="mt-4">
                                        ...
                                        @if ($edit)
                                            <br>
                                            <a href="/agendas/{{ $update->file }}">{{ $update->title }}</a>
                                        @endif
                ...
                @if (!$edit)
                    <div class="col-sm-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">List of Legislative Agenda</h4>
                                    ...
                                    <table class="table user-table">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($agendas as $agenda)
                                                <tr>
                                                    <td>1</td>
                                                    <td><a href="/agendas/{{ $agenda->file }}" target="_blank"
                                                            rel="noopener noreferrer">{{ $agenda->file }}</a></td>
                                                    <td>{{ $agenda->title }}</td>
                                                    <td>{{ $agenda->description }}</td>
                                                    <td>{{ $agenda->remarks }}</td>
                                                    <td
                                                        style="{{ $agenda->is_approved == 1? 'background:blue;color:white': ($agenda->is_approved == 2? 'background:green;color:white': 'background:red;color:white') }}">
                                                        {{ $agenda->is_approved == 1 ? 'Approved' : ($agenda->is_approved == 2 ? 'Pending' : 'Disapproved') }}
                                                    ...
                                                    <td>
                                                        @if ($department == 'super_admin')
                                                            ...
                                                            <a class="btn btn-primary" style="color:white"
                                                                onclick="approve({{ $agenda->id }},{{ $idPage }})">Approve</a>

                                                            <a class="btn btn-danger" style="color:white"
                                                                onclick="disapprove({{ $agenda->id }},{{ $idPage }})">Disapprove</a>
                                                            {{-- @endif --}}
                                                        @else
                                                            <a class="btn btn-primary"
                                                                href="/admin-manage-legislative-file-edit/{{ $agenda->id }}"
    ...
    <script>
        function disapprove(id, idpage) {
            ...
            if (confirm_Final) {
                ...
                window.location.href = "/admin-remarks/" + id + "/" + idpage + "/LegislativeAgendas"
            }
